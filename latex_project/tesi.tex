...
\begin{definition} \gr[]{mhhh these arenot enough assumptions eheh}
  Given an LP problem \eqref{eq:LP}, we say that a weighed aggregation respect to the partitions \(\sigma,\, \omega\) is \emph{structure preserving} if for each \(R \in \sigma\), and for each \(r \in R\), there exists maps \(f^r: \supp(\tilde{A}_R)\to \supp(\tilde{A_r})\), such that \(\tilde A_{R,S} = A_{r,f(S)} \). Where 
\end{definition}
