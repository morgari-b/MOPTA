...
for i in range(4):
    ...
    gl[0,i,:] = GL.loc[GL["Quarter"]=="Q{}".format(i+1)].groupby((GL["Instance"]-1)//4)["Load"].sum().to_list()
    s[i,:] = S.loc[S["Quarter"]=="Q{}".format(i+1)].groupby((S["Instance"]-1)//4)["Generation"].sum().to_list()
    w[i,:] = W.loc[W["Quarter"]=="Q{}".format(i+1)].groupby((W["Instance"]-1)//4)["Generation"].sum().to_list()
el = np.matrix(el)
hl = np.matrix(gl)
es = np.matrix(s)
ew = np.matrix(w)
...
ns = model.addVars(Ns,vtype=GRB.CONTINUOUS, obj=cs,lb=0,ub=5000000)
nw = model.addVars(Nw,vtype=GRB.CONTINUOUS, obj=cw,lb=0,ub=5000000)    
# needed max capacity at electrolyzers z, fuel cells fc
nhz = model.addVars(Nz,vtype=GRB.CONTINUOUS, obj=ch,lb=0,ub=5000000)
nhfc = model.addVars(Nfc,vtype=GRB.CONTINUOUS, obj=ch,lb=0,ub=5000000)
