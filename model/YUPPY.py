...
def ValidateHfix(network,VARS,scenarios, day_initial, scenario_initial):
    ...
    for j in range(scenario_initial,d):
        ...
        if j == scenario_initial: #we start from day day_initial and scenario scenario_initial and then continue as normal
            ...
            Hs[0,:]=H_agg.sel(time=0).max('scenario') # else might get unfeasibility for net 0 solutions
        # start iterating
        for day in pd.date_range('Jan 01 2023','Dec 31 2023',freq='d'):
            if day_num < 365:    
                ...
                c4 = model.addConstrs( delta_H[i,k] >= - H[i,k] + goalH.where(goalH.date.isin(pd.date_range(init_date,freq='h',periods=24)),drop=True).isel(node=k,time=i) for i in range(inst) for k in range(Nnodes))
