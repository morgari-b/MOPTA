...
\begin{definition}
  ...
  \begin{enumerate}
    ...
    \item \(C = \{f^r(C)\}_{_{r\in R}}\) for all \(R \in \rowP_{>1}\). %\(\{f^r(C)\}_{r\in R \in \rowP_{>1}, C \in \colP_{>1}} = \cup_{C \in \colP_{>1}}C\)
  \end{enumerate}
\end{definition}
...
The challenges in constructing a feasible solution are two:
\begin{enumerate}
  \item If \(x_{f^r(C)} \coloneqq \rho_r \tilde{x}_C\) for all \(C \in \supp(\tilde{A}_R)\) and \(C \in \colP_{>1}\). In such cases, \(x\) may not be well defined if \(f^r(C) = f^{r'}(C)\) but \(\rho_r \neq \rho_{r'}\).
  \item If \(x\) is well defined, we have recovered a feasible solution for the constraints in \(\rowP_{>1}\) we need to ensure that \(x\) is also feasible for the remaining constraints in \(\rowP_{=1}\).
\end{enumerate}
 
We call the constraints in \(\rowP \) which are feasible indipendently of how \(\rho\) is picked, \(\rho\)-agnostic and are defined as follows:
\begin{definition}
...
This is a special instance of the following general observation, note that if \(x\) is well defined, then for all \(C \in \supp(A_{r})_{>1}\), we can pick \(R^{(C)} \in \rowP_{>1}\) so that \(C \in \supp(\tilde{A}_{R^{(C)}})\) and \(x_{f^r(C)} = \rho_r \tilde{x}_C\) for all \(r \in R^{(C)}\) and the definition of \(x\) does not depend on the choice of \(R^{(C)}\).
\begin{observation}
  Le \((\rowP,\colP)\) be a structure-preserving, row and column aggregation. For all \(C \in \supp(A_{r})_{>1}\), let \(R^{(C)} \in \rowP_{>1}\) so that \(x_{f^{r'}(C)} \coloneqq \rho_{r'} \tilde{x}_C\) for all \(r' \in R^{(C)}\). A constraint \(r \in \rowP_{=1}\) is \(\rho\)-agnostic if
...
\begin{definition}
  The \emph{hypergraph associated to the aggregation \((\rowP, \colP)\)} is the hypergraph \(\cN, \cE\) having as nodes the aggregated variables \(\cN \coloneqq \colP_{>1}\) and as edges the subsest of \(\cN\) that appear together in not row-agnostic aggregated constraints.
\end{definition}
...
If \((\rowP, \colP)\) is a structure-preserving aggregation and the constraints in \(\rowP_{=1}\) are \(\rho\)-agnostic. Let \(\tilde{x}\) be a solution 
...
Then \(x_{\colP_{=1}} \coloneqq \tilde{x}_{\colP_{=1}}\) and \(x_{f^r(C)} \coloneqq \rho_r \tilde{x}_C\) for all \(C \in \supp(\tilde{A}_R)\) and \(C \in \colP_{>1}\) is well defined and \(x\) is a feasible solution to the unaggregated problem \eqref{eq:LP}.
\end{proposition}
