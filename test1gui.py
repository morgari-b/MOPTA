...
from scenario_generation import read_parameters, SG_beta, SG_weib
...
class ScenarioGenerator(QObject):
    ...
    def run(self):
        # Here, implement the scenario generation logic
        self.progress.emit(f"Generating Wind scenarios for {self.country}...")
        # Simulate processing
        n_scenarios = self.n_scenarios
        params = self.params
        self.EW = SG_weib(n_scenarios, params["wind"][0], params["wind"][1])
        self.progress.emit(f"Wind done, generating PV scenarios for {self.country}...")
        self.ES = SG_beta(n_scenarios, parameters_df = params["PV"][0], E_h = params["PV"][1], night_hours = params["PV"][2], save = False)
        # After processing
        self.progress.emit("Scenarios generated.")
