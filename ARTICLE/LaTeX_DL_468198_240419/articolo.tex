...
\begin{tikzpicture}
  [block/.style={draw,rectangle,minimum height=1.5em,minimum width=3em,fill = red!20, draw= red!50}]
    ...
    \node at (\xzero + 1.4,\yb - 1.4 ) {\Large $-Id$};
    ...
    \node at (\xtwo +2,\yb - 1.4) {\Large $0$};

    
    % Identity block
    \draw[thick] (\xone,\ya) rectangle (\xtwo,0);
    \node at (\xone + 0.5, \ya -1.2 ) {
      $\begin{matrix}
      1 \\ 
      1 \\
      \vdots \\
    ...
    \node at (\xtwo+2.7,\ya-1.2) {$
    \begin{matrix}
      -1 & 0  & 0 & 0 &  0& 0 &  0 & \ldots & 0\\
      -1 & -1  & 0 & 0 & 0 &  0& 0 & \ldots & 0\\
      \vdots & \vdots  & \ \vdots & \vdots& \ \vdots & \vdots & \vdots & \ddots & \vdots\\
      -1 & -1  & -1 & -1 & -1& -1 & -1 & \ldots & -1
    \end{matrix}$};
     % FORTH COLUMN
     \nc{\ytopp}{\ytop + 1/2}
     \draw[thick] (\xthree + 1/6, \ytopp) rectangle (\xthree + 1/2 + 1/6,\yc -1/2);
    %  \node at (\xthree + 1/2  , \ya - \blockheight) {
    %   $
    %   \begin{bmatrix}
    %     ns \\ nw  \\ nh 
    %   \end{bmatrix}$
    ...
    \node at (\xfourr  , \ytopp - 3*\spacefour) {$nh$};
    \node at (\xfourr  , \ytopp - 8*\spacefour) {\Large $\vdots$};
    \node at (\xfourr  , \ytopp - 13*\spacefour) {\Large $\vdots$};
    \node at (\xfourr  , \ytopp - 18*\spacefour) {\Large $\vdots$};
     % FIFTH COLUMN
     \node at (\xthree + 1  , \ya - \blockheight) {\Large $\geq$};

    % SIXTH COLUMN
    \nc{\xsix}{\xfourr + 0.8};
    \draw[thick] (\xsix, \ytop) rectangle (\xsix + 1/2,\yc -1/2);
    \node at (\xsix + 1/4 , \ytop - \spacefour) { \small $HL$};
    \node at (\xsix + 1/4 , \ytop - 2*\spacefour) {\small $EL$};
    \node at (\xsix + 1/4  , \ytop - 8*\spacefour) {\Large $\vdots$};
    \node at (\xsix + 1/4  , \ytop - 13*\spacefour) {\Large $0$};
    \node[rotate=-90] at (\xsix + 1/4 , \ytop - 22*\spacefour) {max CEP};
