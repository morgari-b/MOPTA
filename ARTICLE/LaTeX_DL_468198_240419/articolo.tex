...
The parameters $ES, EW, EL, HL$, indexed by scenario, time step and node represent the time series of power generation and load values for different scenarios in every node of the grid. They are generated through the method described in \textcolor{pink}{the appendix \ref{generation}}\\
...
\begin{tabularx}{\textwidth}{ccl}
  ...
  \noalign{\smallskip}\hline\noalign{\smallskip}
    ...
    ch\_t$_n$ & \euro/(kg$\cdot$\textcolor{pink}{h}) & Cost of storing hydrogen for one time step \\
...
\begin{align*}
    ...
    &  + 0.033\hspace{-2em}& \cdot f&hte_k \cdot \text{HtE}_{j,t,n} - \text{EtH}_{j,t,n}\ + \\
    ...
    &&& + 30 \cdot feth_k \cdot \text{EtH}_{j,t,n} - \text{HtE}_{j,t,n} \ + \\
...
\begin{align*}
    \text{Net Transfer Capacity:} \quad & \text{P\_edge}^\pm_{j,t,l}\le\text{NTC}_l + \text{addNTC}_l ;\\
    \text{Hydrogen Transfer Capacity:} \quad & \text{H\_edge}^\pm_{j,t,l}\le\text{MH}_l + \text{addMH}_l.
\end{align*}
