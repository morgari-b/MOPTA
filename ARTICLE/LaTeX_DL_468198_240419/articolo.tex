...
&= \rho_r (\tilde{b} - \tilde{A}_{\colP_{=1}} \tilde{x}_{\colP_{=1}}) \\
&= \frac{b_r - A_{r, \colP_{=1}} \tilde{x}_{\colP_{=1}}}{\rho_r}
\end{align*}
...
Thus we have:
% \begin{obs}
% If \((\rowP, \colP)\) is a structure-preserving aggregation, 
% let \(R \in \rowP\) and \(r \in R\). Let \(\tilde{x}\) be a solution 
...
% Otherwise, define \(\rho_r \coloneqq \frac{b_r - A_{r, \colP_{=1}} \tilde{x}_{\colP_{=1}}}{\tilde{b}_r - \tilde{A}_{R, \colP_{=1}} \tilde{x}_{\colP_{=1}}}\). If \(rho_r \geq 0\) and is constant over the connected components of the hypergraph associated to \ref{eq:LP}. 
% Then \(x_{\colP_{=1}} \coloneqq \tilde{x}_{\colP_{=1}}\) and \(x_{f^r(C)} \coloneqq \rho_r \tilde{x}_C\) for all \(C \in \supp(\tilde{A}_R)\) and \(C \in \colP_{>1}\) is well
% \end{obs}
