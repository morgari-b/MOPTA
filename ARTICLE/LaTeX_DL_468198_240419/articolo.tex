...
\begin{itemize}
  ...
  \item For \(\rho_r\) constraints which behave well under generalisez convex combinations of variables can be excluded.
\end{itemize}
}
...
\textcolor{violet}{
Altre cose da considerare di aggiungere a questa definizione: \(f^{r'}(C') = f^r(C) \implies C=C'\) e \(\cup_{r\in R \in \rowP_{>1}} = \cup_{C \in \colP_{>1}}C\)
...
\begin{proof}
  ...
  \] 
\end{proof}
From this observations we have a class of constraints, which however \(\rho\) is fixed remain valid, and thus the corresponding edges can be removed from the hypergraph associated to the aggregation.
That is, if we define \(\rho\) on the hypergraph obtained by removing edges, then the corresponding \(x\) will be feasible for this class of constraints.
\begin{observation}
  If \(\colP\) is a constant-coefficients column aggregation, and \((\rowP, \colP)\) a constant weight aggregation such that \(f^{r'}(C') = f^r(C) \implies C=C'\) e \(\cup_{r\in R \in \rowP_{>1}} = \cup_{C \in \colP_{>1}}C\). For \(r \in \rowP_{=1}\) such that for all \(R \in \rowP_{>1}\) and for all 
  \(C \in \colP_{>1}\) such that \(C \cap \supp(A_{r,c}) \neq \emptyset\), \(A_{r,f^{r'}(C)} = \rho_{r'}\) for all \(r' \in R\) with \(R^{(C)} \in \rowP_{>1}\) such that \(C \in \supp(A_{R^{(C)}})\)
  then if \(\rho\) is such that \( \rho_R \in \bR\) for all \(R \in \rho_{>1}\), we have \(\rho_Rx_R = 1\), then \(A_rx=b_r\).
\end{observation}
\begin{proof}
  \[
...
We refer to these constraints a \emph{\(\rho\) agnostic constraints}. We can now consider the hypergraph as defined before without the row agnostic constraints.
