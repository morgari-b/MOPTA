...
\begin{observation}
  Under the hypothesis of the previous proposition, if \(\tilde{C}_C = c_{f(r,C)}\) and if \( f^r(C) = f^{r'}(C')\) then \(C = C'\) and \(\{f^r(X)\}_{r \in \cup_{R\in \rowP_{>1}} = \cup_{C \in \colP_{>1}}C}\). then the cost of the aggregated problem 
...
Then if the weights of the columns correspond to the coefficients of the variables in the aggregated problem (We substitute the columns corresponding to \( C \) with a vector of all ones). The previous conditions ensures that such weights are well defined and do not depend on the non-null row considered.
We refer to this as a \emph{constant-coefficients column agreggation}

\begin{observation}
Under the hypothesis of the previous two observations and is a constant-coefficients column aggregation, then the relaxation is exact. Thas is \(x\) is an optimal solution of the original problem \eqref{eq:LP}. \gr[]{da scrivere meglio ma devo fuggire}
\end{observation}
